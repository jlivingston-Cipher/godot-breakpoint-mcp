...
Launch a C# Godot game under netcoredbg. `program` defaults to the configured Mono/.NET Godot binary and `args` to `['--path', <C# project>]`; override either to debug a different .NET program. Buffered breakpoints are applied during the handshake. **Refuses when the runtime bridge port is already bound — but only when launching the configured Godot binary.** Pointing `program` at some other .NET program is never gated: it has no Breakpoint autoload and no interest in the runtime port, so refusing there would be a check firing when nothing is wrong.
- **Input**
```json
