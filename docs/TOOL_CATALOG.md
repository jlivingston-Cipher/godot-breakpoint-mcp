...
🔴 **This call puts the editor back on the scene you had open, and `edited_scene` names where it actually landed.** The decomposition above starts at `scene.new`, whose addon implementation calls `EditorInterface.open_scene_from_path`, so partway through this call the editor is editing the file being written. Measured against a live Godot 4.7 on the default tool surface, before the restore existed: compose a table, create a card template, then call `card_instance` with `parent: "."` exactly as its own description invites — the card was created, its slots bound, `isError` was false, and it was in the CARD TEMPLATE while the table went untouched and every call answered success. The restore closes that: the scene being edited when the call arrives is reopened once the write is saved, so `card_instance`, `piece_instance`, `board_place`, `board_tile_place` and the `interact_*` tools act on what you were composing with no intervening `scene_open`. **The restore is best effort and the field is the check** — `edited_scene` is read from the editor after the reopen rather than echoed from `path`, so an editor that held no scene to begin with, or one whose previous scene file has since been deleted, reports a path that is not yours instead of a silent success.
- **Input**
```json
