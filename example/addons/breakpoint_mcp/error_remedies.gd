...
const EDITOR_REMEDIES := {
	...
	"unknown_method": "Re-run `breakpoint-mcp init` to refresh the addon — the copy installed in the project is older than the host.",
...
const RUNTIME_REMEDIES := {
	...
	"unknown_method": "Re-run `breakpoint-mcp init` and restart the project — the addon running in the game is older than the host.",
