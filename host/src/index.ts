...
async function main(): Promise<void> {
  ...
  const server = new McpServer(
    { name: "breakpoint-mcp", version: "1.22.0" },
  ...
  // every group → the full 289-tool surface, byte-identical to before. A filter
  ...
  let peers: { stopAll: () => void } | undefined;
  ...
  // via prompts/list). Adds NO tools — the 289-tool count is unchanged — and drives
  ...
  const shutdown = () => {
    ...
    csDap.close();
    processes?.killAll();
    // F6: headless peers are children of this server — never outlive it.
    peers?.stopAll();
    process.exit(0);
  };
