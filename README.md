...
>
> **npm 1.85.1 · addon 1.16.0 · full 293 / secure-default 280 tools · 6 MCP resources · MIT.** The host builds against
> the stable `@modelcontextprotocol/sdk` 1.x API and is exercised by a 1,000-test suite plus
...
default surface is **280 tools an agent can actually call**, not 293 with a warning label on
...
| Published risk annotations + machine-readable tool export | ✗ | ✅ all 293 tools, CI-enforced |
...
alternatives in the Asset Library. The tool *count* is not the axis — ours is 293 because that
...
reduction**, and it is one environment variable:
...
Breakpoint MCP is organized into four capability **planes** (full **293 tools**, or **280** with the privileged capability group off by default — see [Safety & trust model](#safety--trust-model) — plus **6 resources**):
...
| `BREAKPOINT_PRIVILEGED_GROUPS` | *(unset → none)* | Comma/space list of the default-OFF capability groups to enable: `code-execution`, or `all`. Off → secure-default **280** tools; opting in loads the **full 293** — see [Safety & trust model](#safety--trust-model) |
...
By default the server registers the full surface (293 tools). On Claude Code that costs
nothing at decision time: **Tool Search** defers the catalog and loads each schema on demand
(a measured **~86–98% upfront token reduction** — the model never sees all 293 at once). For
...
Examples: `BREAKPOINT_TOOLSETS=c` → 27 runtime tools; `a,b` → 156 (editor + CLI);
...
| `BREAKPOINT_TOOLSETS=b,c,d` | → **93** | The above plus the headless CLI (Plane B) and the running-game runtime and verification family (Plane C) — everything except live editor authoring |
...
misapply. Recipes also add no tools (the count stays 293) and cost nothing until pulled.
...
- **Least-privilege by default — capability groups (since 1.18.0).** The higher-blast tools are
  ...
  secure-default surface is **280 tools**; opt in with
  `BREAKPOINT_PRIVILEGED_GROUPS=code-execution` (or `all`, or
  `breakpoint-mcp init --trust full`) to load the full **293**. There is deliberately no `network`
