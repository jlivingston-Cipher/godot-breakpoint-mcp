...
PROSE_NUMERAL_EXEMPT: dict[tuple[str, str], str] = {
    ("README.md", "| Google Antigravity | 100 |"): (
        "🆕 289 — A CLIENT'S OWN TOOL-LIST CAP. Nothing in this repository decides how many tools Google Antigravity or VS Code will accept, so no derivation here can produce the number and a derivation that appeared to would be wrong rather than lenient — `An authoring-focused server we re-measured`'s argument, one field over. It is not unowned: `host/src/client-caps.ts` carries each cap with the issue it was read from and the date, and `host/test/client_caps.test.ts` joins this table to that module in BOTH directions — a cap the module carries that the table omits is a client we know about and do not warn for, and a table row the module does not carry is a number a user acts on that nothing re-checks. Keyed by the sentence, so the day a cap is re-read against a different client the exemption stops matching and this check says so. The canonical table's Antigravity row; the quoted string is the refusal a user searches for, which is the whole reason the section exists."
    ),
    ("README.md", "| VS Code (agent mode) | 128 |"): (
        "🆕 289 — A CLIENT'S OWN TOOL-LIST CAP. Nothing in this repository decides how many tools Google Antigravity or VS Code will accept, so no derivation here can produce the number and a derivation that appeared to would be wrong rather than lenient — `An authoring-focused server we re-measured`'s argument, one field over. It is not unowned: `host/src/client-caps.ts` carries each cap with the issue it was read from and the date, and `host/test/client_caps.test.ts` joins this table to that module in BOTH directions — a cap the module carries that the table omits is a client we know about and do not warn for, and a table row the module does not carry is a number a user acts on that nothing re-checks. Keyed by the sentence, so the day a cap is re-read against a different client the exemption stops matching and this check says so. The same table's VS Code row — a cap across every enabled server rather than over ours alone, which is why it is stated and not derived."
    ),
    ("docs/USER_GUIDE.md", "caps the list at 100 and VS Code blocks agent mode at 128"): (
        "🆕 289 — A CLIENT'S OWN TOOL-LIST CAP. Nothing in this repository decides how many tools Google Antigravity or VS Code will accept, so no derivation here can produce the number and a derivation that appeared to would be wrong rather than lenient — `An authoring-focused server we re-measured`'s argument, one field over. It is not unowned: `host/src/client-caps.ts` carries each cap with the issue it was read from and the date, and `host/test/client_caps.test.ts` joins this table to that module in BOTH directions — a cap the module carries that the table omits is a client we know about and do not warn for, and a table row the module does not carry is a number a user acts on that nothing re-checks. Keyed by the sentence, so the day a cap is re-read against a different client the exemption stops matching and this check says so. The configuration section's one-line statement of both caps, written on one line deliberately so one exemption owns both numerals."
    ),
    ("docs/USER_GUIDE.md", "Antigravity's refusal reads"): (
        "🆕 289 — A CLIENT'S OWN TOOL-LIST CAP. Nothing in this repository decides how many tools Google Antigravity or VS Code will accept, so no derivation here can produce the number and a derivation that appeared to would be wrong rather than lenient — `An authoring-focused server we re-measured`'s argument, one field over. It is not unowned: `host/src/client-caps.ts` carries each cap with the issue it was read from and the date, and `host/test/client_caps.test.ts` joins this table to that module in BOTH directions — a cap the module carries that the table omits is a client we know about and do not warn for, and a table row the module does not carry is a number a user acts on that nothing re-checks. Keyed by the sentence, so the day a cap is re-read against a different client the exemption stops matching and this check says so. The quoted refusal text. A numeral inside a quotation belongs to whoever wrote the quotation — the day we 'correct' it, it stops being one."
    ),
    ("docs/USER_GUIDE.md", "The client never loads the server at all"): (
        "🆕 289 — A CLIENT'S OWN TOOL-LIST CAP. Nothing in this repository decides how many tools Google Antigravity or VS Code will accept, so no derivation here can produce the number and a derivation that appeared to would be wrong rather than lenient — `An authoring-focused server we re-measured`'s argument, one field over. It is not unowned: `host/src/client-caps.ts` carries each cap with the issue it was read from and the date, and `host/test/client_caps.test.ts` joins this table to that module in BOTH directions — a cap the module carries that the table omits is a client we know about and do not warn for, and a table row the module does not carry is a number a user acts on that nothing re-checks. Keyed by the sentence, so the day a cap is re-read against a different client the exemption stops matching and this check says so. The troubleshooting heading. The error string is IN the heading on purpose: a user who searches the message their editor printed lands on the section that fixes it."
    ),
    ("docs/USER_GUIDE.md", "Google Antigravity stops at 100"): (
        "🆕 289 — A CLIENT'S OWN TOOL-LIST CAP. Nothing in this repository decides how many tools Google Antigravity or VS Code will accept, so no derivation here can produce the number and a derivation that appeared to would be wrong rather than lenient — `An authoring-focused server we re-measured`'s argument, one field over. It is not unowned: `host/src/client-caps.ts` carries each cap with the issue it was read from and the date, and `host/test/client_caps.test.ts` joins this table to that module in BOTH directions — a cap the module carries that the table omits is a client we know about and do not warn for, and a table row the module does not carry is a number a user acts on that nothing re-checks. Keyed by the sentence, so the day a cap is re-read against a different client the exemption stops matching and this check says so. The same section's body, restating both caps for a reader who arrived at the heading from a search engine rather than from the guide."
    ),
