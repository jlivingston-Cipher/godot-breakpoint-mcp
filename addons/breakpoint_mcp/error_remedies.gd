...
const EDITOR_REMEDIES := {
	...
	"window_not_drawing": "Make the editor window visible again — unminimise it, switch to its desktop or Space, or uncover it — then call again. `main_screen_set` cannot help: the engine, not one tab, has stopped drawing.",
...
const RUNTIME_REMEDIES := {
	...
	"window_not_drawing": "Make the game's window visible again — unminimise it, switch to its desktop or Space, or uncover it — then call again. `runtime_step_frames` cannot help: the frame advances but nothing is presented.",
